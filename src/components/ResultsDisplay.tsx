  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <tbody>
          {result.perCity.map((city, i) => (
            <tr key={`${city.cityId}-${i}`} className="border-b border-slate-100 last:border-0">
              <td className="px-4 py-3 font-medium text-slate-800">{city.cityName}</td>
              <td className="px-4 py-3 text-right text-slate-600">{city.nights}</td>
              <td className="px-4 py-3 text-right text-slate-600">
                {city.transitInMin > 0 ? formatMin(city.transitInMin) : '—'}
              </td>
      ...
      <p className="px-4 py-2.5 text-xs text-slate-400">
        ...
        Times are typical-schedule estimates including door-to-door overhead, not guarantees.
      </p>
    </div>
  );
}
